from __future__ import annotations

import pandas as pd

from buffmini.stage23.order_builder import build_adaptive_orders


def _frame() -> pd.DataFrame:
    return pd.DataFrame(
        {
            "timestamp": pd.date_range("2026-01-01", periods=6, freq="1h", tz="UTC"),
            "open": [100.0, 100.5, 101.0, 100.8, 100.4, 100.2],
            "high": [100.8, 101.0, 101.4, 101.0, 100.8, 100.5],
            "low": [99.8, 100.1, 100.5, 100.2, 99.9, 99.7],
            "close": [100.0, 100.5, 101.0, 100.8, 100.4, 100.2],
            "volume": [5000.0, 5200.0, 5300.0, 5100.0, 4900.0, 5000.0],
            "atr_14": [1.2, 1.2, 1.1, 1.1, 1.0, 1.0],
        }
    )


def _cfg(mode: str) -> dict:
    return {
        "costs": {"round_trip_cost_pct": 0.1, "slippage_pct": 0.0005},
        "cost_model": {"mode": "simple", "round_trip_cost_pct": 0.1},
        "risk": {"max_gross_exposure": 5.0},
        "evaluation": {
            "stage10": {"evaluation": {"take_profit_atr_multiple": 3.0}},
            "stage23": {
                "enabled": True,
                "order_builder": {
                    "allow_size_bump_to_min_notional": False,
                    "min_trade_notional": 10.0,
                },
            },
            "stage24": {"enabled": False},
            "constraints": {
                "mode": str(mode),
                "live": {
                    "min_trade_notional": 10.0,
                    "min_trade_qty": 0.0,
                    "qty_step": 0.001,
                },
                "research": {
                    "min_trade_notional": 1.0,
                    "min_trade_qty": 0.0,
                    "qty_step": 0.001,
                },
            },
            "modes": {
                "research": {
                    "min_notional_override": 1.0,
                    "ignore_exchange_precision": True,
                    "enforce_margin_caps": False,
                    "enforce_min_notional": False,
                    "enforce_size_step": False,
                },
                "live": {
                    "use_exchange_rules": True,
                },
            },
        },
    }


def test_stage27_9_research_mode_accepts_and_flags_live_infeasible() -> None:
    frame = _frame()
    raw_side = pd.Series([1, 1, 1, 1, 1, 1], index=frame.index, dtype=int)
    score = pd.Series([0.01, 0.012, 0.011, 0.013, 0.01, 0.009], index=frame.index, dtype=float)
    out = build_adaptive_orders(
        frame=frame,
        raw_side=raw_side,
        score=score,
        cfg=_cfg("research"),
        symbol="BTC/USDT",
    )
    assert int((out["accepted_signal"] != 0).sum()) > 0
    flags = list(out.get("research_infeasible_flags", []))
    assert flags
    assert all(bool(item.get("research_infeasible_live", False)) for item in flags)
    reasons = {str(item.get("reason", "")) for item in flags}
    assert "SIZE_TOO_SMALL" in reasons


def test_stage27_9_live_mode_enforces_exchange_rules() -> None:
    frame = _frame()
    raw_side = pd.Series([1, 1, 1, 1, 1, 1], index=frame.index, dtype=int)
    score = pd.Series([0.01, 0.012, 0.011, 0.013, 0.01, 0.009], index=frame.index, dtype=float)
    out = build_adaptive_orders(
        frame=frame,
        raw_side=raw_side,
        score=score,
        cfg=_cfg("live"),
        symbol="BTC/USDT",
    )
    assert int((out["accepted_signal"] != 0).sum()) == 0
    reasons = [str(item.get("reason", "")) for item in out.get("reject_events", [])]
    assert "SIZE_TOO_SMALL" in reasons
    assert list(out.get("research_infeasible_flags", [])) == []

