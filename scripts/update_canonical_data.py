...
def _fetch_incremental(
    *,
    ...
    while cursor <= int(end_ms):
        batch = list(exchange.fetch_ohlcv(symbol=market_symbol, timeframe=timeframe, since=cursor, limit=int(limit)))
        batch_count += 1
        if not batch:
            break
        for row in batch:
...
def main() -> None:
    ...
    start_ts = now - pd.Timedelta(days=int(args.years) * 365)
    ...
    for symbol in symbols:
        ...
        if not existing.empty:
            last = int(pd.to_datetime(existing["timestamp"], utc=True, errors="coerce").dropna().iloc[-1].timestamp() * 1000)
            since_ms = max(start_ms, last + 60_000)

        fetched = _fetch_incremental(
            exchange=exchange,
        ...
        merged = prepare_frame(pd.concat([existing, fetched], ignore_index=True))
