"""CLI wrapper for Stage-23 baseline-vs-enabled forensic audit."""
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Run Stage-23 choke audit (baseline vs enabled)")
...
def main() -> None:
    ...
    print(
        ...
        f"{float(payload['baseline']['metrics']['walkforward_executed_true_pct']):.6f} -> "
        f"{float(payload['after']['metrics']['walkforward_executed_true_pct']):.6f}"
    )
